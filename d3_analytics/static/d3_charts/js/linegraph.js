function draw_line_graph(json_data) {
    ...
    if(time_dependent) {
       ...
       var xScale = d3.scale.ordinal()
            .domain(xticks)
            .rangeRoundBands([0, svgWidth-margin.right-margin.left], 1)
    }
    ...
    if(time_dependent) {
        var lineFunction = d3.svg.line()
            .x(function(d) { return xScale(parseDate(d.x_value)); })
    ...
    svg.append("path")
            ...
            .attr("transform", "translate(" + margin.left  + ", " + margin.top + ")")
            .attr("d", lineFunction(json_data["data_points"]));
    ...
    // style for dots on hover
    var dots_activated_css = {
        "stroke": "#72b269",
        "stroke-width": "1.5",
        "fill": "#fff"
    }
    ...
    var dots = svg.append("g")
          ...
          .enter()
            ...
            .attr("cx", function(d, i) {
                if(time_dependent) {
                    return xScale(parseDate(d.x_value));
                }
                else {
                    return xScale(i);
                }
            ...
            .on("mouseover", function(d) {
                // reset any activated dots
                dots.attr("r", 4).style(dots_css);

                // apply fresh transition on dots
                d3.select(this).attr("r", 6)
                    .style("fill", dots_css["fill"])
                  .transition()
                    .duration(1000)
                    .attr("r", 4)
                    .style(dots_activated_css);

                tip.show(d);

            })
            .on("mouseout", function(d) {
                d3.select(this).attr("r", 4).transition()
                    .duration(1000)
                    .style(dots_css);
    ...
    // reset broken transitions on grid hover
    svg.selectAll(".grid").on("mouseover", function(d) {
                dots.attr("r", 4).transition()
                    .duration(1000)
                    .style(dots_css);
            })
            .on("mouseout", function(d) {
